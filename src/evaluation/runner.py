...
class EvaluationRunner:
    def __init__(self, config: ExperimentConfig, model_configs: list[ModelConfig], group: str,
        ...
        if not self.output_dir.is_absolute():
            self.output_dir = _PROJECT_ROOT / self.output_dir
    ...
    def run(
        ...
        for bm_name in benchmarks:
            ...
            for mcfg in self.model_configs:
                ...
                safe_name = mcfg.name.replace("/", "__")
                ...
                for rep in range(repeats):
                    run_id = f"{bm_name}_{safe_name}_{stage}_r{rep}"
                    raw_path = self.output_dir / "raw" / bm_name / safe_name / f"{run_id}.jsonl"
                    parsed_path = self.output_dir / "parsed" / bm_name / safe_name / f"{run_id}.jsonl"
            ...
            for future in as_completed(futures):
                ...
                if record.get("error"):
                    ...
                    if error_count >= self._max_consecutive_errors:
                        logger.error(
                            f"[{model_name}] {self._max_consecutive_errors} consecutive errors, skipping remaining items"
                        )
    ...
    def _load_dataset(self, benchmark: str, bm_config: dict) -> BaseBenchmarkDataset:
        ...
        # Resolve relative paths against the project root (capacity_eval/)
        if not Path(data_dir).is_absolute():
            data_dir = str(self._project_root / data_dir)
